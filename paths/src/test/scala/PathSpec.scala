...
class PathSpec extends FlatSpec with Matchers {
  ...
  it should "find the best path" in {
    val segments = Vector(Segment(tl, tr), Segment(tr, br), Segment(tl, bl), Segment(bl, br))

    helper.bestPath(segments, tl, br).get shouldBe Path(Vector(Segment(tl, tr), Segment(tr, br)))
  }

  it should "find the best path with a stop" in {
    val segments = Vector(Segment(tl, tr), Segment(tr, br), Segment(tl, bl), Segment(bl, br))

    helper.bestPath(segments, tl, br, Vector(bl)).get shouldBe Path(Vector(Segment(tl, bl), Segment(bl, br)))
  }
