...
class MoviesUtilSuite extends FlatSpec with Matchers {
  ...
  "9 movies" should "be from 1995" in {
    findTitlesByYear("1995") should contain theSameElementsAs Vector(
      "Se7en (1995)",
  ...
  "3 movies" should "have rating >= 9" in {
    findMoviesWithRatingAtLeast(9) should contain theSameElementsAs Vector(
  ...
  "DiCaprio and Tom Hanks" should "have played together only in Catch Me ..." in {
    moviesWithAll(Vector("DiCaprio, Leonardo", "Hanks, Tom")) should contain theSameElementsAs Vector(
      Movie("Catch Me If You Can (2002)", 8f))
  }

  "11 movies" should "have featured Tom Hanks or DiCaprio" in {
    moviesWithAny("DiCaprio, Leonardo", "Hanks, Tom") should have size 11
  }
  ...
  "max rating for Matt Damon movies" should "be slightly lower than for DiCaprio" in {
    maxRating("DiCaprio, Leonardo") shouldBe 8.7f
    maxRating("Damon, Matt") shouldBe 8.5f
  }

  "average rating for Matt Damon movies" should "be slightly better than for DiCaprio" in {
    avgRating("DiCaprio, Leonardo") shouldBe 8.3f
    avgRating("Damon, Matt") shouldBe 8.34f
  }
